...
If we do not look at `f`, and do not know what it did to `data`, we can not confirm what will be printed. But if `data` is _Immutable_, you can be sure that `data` haven't changed and `value` is printed:
